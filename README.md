...
전체 작업 시간은 2~3일 정도 소요되었고 총 소요 시간은 32시간 정도입니다.

| 작업 구분 | 소요 시간 | 내용 |
|---|---:|---|
| 프로젝트 구조 설계 | 약 3시간 | Next.js App Router 구조, Prisma 모델, 주요 페이지 흐름 설계 |
| 인증 기능 구현 | 약 3시간 | 회원가입, 로그인, 로그아웃, 사용자와 회사 소속 정보 처리 |
| 배출 데이터 기능 구현 | 약 5시간 | 배출 데이터 등록, 조회, 수정, 삭제 및 배출량 자동 계산 구현 |
| 감축 목표 기능 구현 | 약 3시간 | 목표 등록, 수정, 삭제 및 대시보드 목표 대비 사용률 계산 |
| 보고서 기능 구현 | 약 4시간 | 기간별 보고서 생성, 목록 조회, 상세 조회 기능 구현 |
| UI 스타일 개선 | 약 6시간 | EcoTrack 브랜드 적용, 사이드바 레이아웃, 카드형 대시보드, 로그인/회원가입 화면 정리 |
| 오류 수정 및 테스트 | 약 5시간 | Hydration 오류, formData name 불일치, 권한 처리, 로그아웃 드롭다운 클릭 문제 해결 |
| README 및 제출 자료 정리 | 약 3시간 | README 작성, 스크린샷 추가, 시연 영상 제작 및 정리 |

## 시간이 많이 소요된 부분

### 1. Hydration 오류 해결

대시보드 화면에서 HTML 태그 중첩 구조가 맞지 않아 Hydration 오류가 발생했습니다.
조건부 랜더링 부분에서 p 태그 안에 다시 p 태그가 들어가는 구조를 수정하여 서버에서 랜더링된 HTML과 클라이언트 렌더링 결과가 일치하도록 정리했습니다.

### 2. 월별 배출량 차트 실제 데이터 반영

초기에는 임시 배열을 사용해 차트를 표시했지만, 실제 배율 데이터의 activityDate와 emissionAmount를 기준으로 월별 합계를 계산하도록 수정했습니다.
이를 통해 대시보드에서 실제 월별 배출량 추이를 확인할 수 있도록 개선했습니다.

### 3. 공통 UI 스타일 정리

처음에는 각 페이지에서 inline style을 사용했지만, 화면이 늘어나면서 스타일 관리가 어려워졌습니다.
이후 global.css에 공통 클래스 기반 스타일을 만들고, 대시보드, 배출 데이터, 감축 목표, 보고서, 로그인/회원가입 화면을 같은 디자인 톤으로 통일했습니다.
